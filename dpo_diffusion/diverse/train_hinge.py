...
def run_train(train_dataset,
    ...
    def loss_sim_fn(img_1, img_2, target):
        sim = F.cosine_similarity(img_1, img_2, dim=1)
        loss = 1 - sim
    ...
    if get_rank() == 0:
        model.eval()
        valid_loss_sim = 0
        labels_true, labels_sim = [], []
        with torch.no_grad():
            for step, batch_data_package in enumerate(valid_loader):
                emb_img_1, emb_img_2 = model(batch_data_package)
        ...
        print('Validation - Iteration %d | SimLoss %6.5f | BootsAcc %6.4f | Pearson %6.4f' %
              (0, valid_loss_sim, sim_boots_acc, pearson_r))
        writer.add_scalar('Validation-Loss', valid_loss_sim, global_step=0)
        writer.add_scalar('Validation-BootsAcc', sim_boots_acc, global_step=0)
...
#                    writer.add_scalar('Train-Acc', sum(acc_list) / len(acc_list), global_step=train_iteration)
            ...
            if (iterations % steps_per_valid) == 0:
                if get_rank() == 0:
                    ...
                    if sim_boots_acc > best_acc:
                        print("Best BootsAcc so far. Saving model")
                        best_acc = sim_boots_acc
    ...
    if get_rank() == 0:
        ...
        print('Test Loss %6.5f | Acc %6.4f | Pearson %6.4f' % (test_loss, sim_boots_acc, pearson_r))
